...
export default function CategoriesClient({ categories }: { categories: any[] }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-primary/10 p-6 sm:p-8 rounded-2xl border border-primary/20 relative overflow-hidden">
        ...
        <Link 
          ...
          className="relative z-10 inline-flex items-center justify-center bg-primary text-white font-bold px-5 py-3 rounded-xl shadow-lg shadow-primary/20 hover:scale-105 hover:bg-emerald-600 transition-all gap-2 text-sm"
        >
                ...
                <tr key={cat.id} className="hover:bg-primary/5 transition-colors group">
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="flex justify-end gap-2">
                       <Link href={`/admin/categories/${cat.id}/edit`} className="h-9 w-9 rounded-lg bg-muted text-foreground/70 flex items-center justify-center hover:bg-primary hover:text-white transition-all" title="Edit">
                       ...
                       <button 
                         ...
                         title="Delete"
                       >
                         <Trash2 className={`h-4 w-4 ${deleting === cat.id ? "animate-spin" : ""}`} />
                       </button>
                    </div>
                  </td>
                </tr>
