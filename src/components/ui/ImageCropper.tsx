...
export function ImageCropper({ file, onCrop, onCancel, aspectRatio = 16 / 9 }: ImageCropperProps) {
  ...
  // Use portal to render outside of any parent <form>
  return createPortal(
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={(e) => e.stopPropagation()}>
      <div className="bg-card rounded-2xl shadow-2xl border border-border max-w-[700px] w-full overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4 flex flex-col items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={zoomOut} className="p-2 rounded-lg bg-muted hover:bg-muted/80 transition-colors" title="Zoom Out">
              <ZoomOut className="h-5 w-5" />
            </button>
            <span className="text-sm font-bold text-foreground/60 w-16 text-center">{Math.round(scale * 100)}%</span>
            <button type="button" onClick={zoomIn} className="p-2 rounded-lg bg-muted hover:bg-muted/80 transition-colors" title="Zoom In">
              <ZoomIn className="h-5 w-5" />
            </button>
            <button type="button" onClick={resetPosition} className="p-2 rounded-lg bg-muted hover:bg-muted/80 transition-colors ml-2" title="Reset">
        ...
        <div className="flex justify-end gap-3 p-4 border-t border-border">
          <button type="button" onClick={onCancel} className="px-5 py-2.5 rounded-xl border border-border font-bold text-sm hover:bg-muted transition-colors">
            Cancel
          </button>
          <button type="button" onClick={handleCrop} className="px-5 py-2.5 rounded-xl bg-primary text-white font-bold text-sm hover:bg-emerald-600 transition-colors flex items-center gap-2">
